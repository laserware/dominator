import { isElementOrDocument, isEvent } from "./internal/typeGuards";
import type { ElemInput, TargetType } from "./types";
 ...
 *   const parent = dom.element<HTMLDivElement>(childElement.parentElement);
 * }
 */
export function element<T extends Element = HTMLElement>(
  input: ElemInput | null,
  ...
  if (isEvent(input)) {
    if (targetType === "currentTarget") {
      return (input?.currentTarget as unknown as T) ?? null;
    } else if (targetType === "target") {
      return (input?.target as unknown as T) ?? null;
    } else {
